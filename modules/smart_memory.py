...
def import_from_text(raw_text: str) -> list[dict]:
    ...
    for line in lines:
        ...
        # Normalize numeric ranges before further processing.
        # "5–6" / "5-6" / "5—6" between digits → "5 to 6"
        # This prevents "5–6 years old" being read as "56 years old" by the AI.
        clean = re.sub(r'(\d+)\s*[–—]\s*(\d+)', r'\1 to \2', clean)
        # ASCII hyphen ONLY between digits (not in compound words or bullets)
...
# Parent hub node each category links back to (for graph edges)
_OBS_HUB = {
    "profile":     "User",
    "instruction": "Instructions",
    "automation":  "Automations",
...
# Sub-hub display names for hub node headers
_OBS_HUB_DESC = {
    "User":         "User profile memories — personal facts, preferences, background.",
    "Instructions": "Behavioral instructions — directives that modify iZACH responses globally.",
    "Automations":  "Scheduled automation memories — recurring tasks and cron jobs.",
    "Projects":     "Project notes and work-in-progress items.",
}


def _ensure_hub_nodes():
    """Create/update hub .md files so the Obsidian graph has a skeleton."""
    try:
        from modules.obsidian_brain import VAULT_PATH

        mem_root = os.path.join(VAULT_PATH, "Memory")
        os.makedirs(mem_root, exist_ok=True)

        # ── iZACH Brain hub ────────────────────────────────────────────
        brain_md = (
            "---\ntags: [hub, izach-brain]\n---\n\n"
            "# iZACH Brain\n\n"
            "Central knowledge graph for iZACH smart memory.\n\n"
            "## Sub-hubs\n\n"
            "- [[User]]\n"
            "- [[Instructions]]\n"
            "- [[Automations]]\n"
            "- [[Call-Messages-Logs]]\n"
            "- [[Projects]]\n\n"
            "---\n*Auto-generated by iZACH Smart Memory*\n"
        )
        with open(os.path.join(mem_root, "iZACH Brain.md"), "w", encoding="utf-8") as f:
            f.write(brain_md)

        # ── Sub-hub nodes ───────────────────────────────────────────────
        for hub, desc in _OBS_HUB_DESC.items():
            folder = os.path.join(mem_root, hub)
            os.makedirs(folder, exist_ok=True)
            hub_md = (
                f"---\ntags: [hub, {hub.lower()}, izach-memory]\n---\n\n"
                f"# {hub}\n\n"
                f"{desc}\n\n"
                f"Linked from: [[iZACH Brain]]\n\n"
                f"---\n*Auto-generated by iZACH Smart Memory*\n"
            )
            with open(os.path.join(folder, f"{hub}.md"), "w", encoding="utf-8") as f:
                f.write(hub_md)

        # ── Call/Messages Logs stub ──────────────────────────────────────
        logs_dir = os.path.join(mem_root, "Call-Messages-Logs")
        os.makedirs(logs_dir, exist_ok=True)
        logs_md = (
            "---\ntags: [hub, logs, izach-memory]\n---\n\n"
            "# Call & Messages Logs\n\n"
            "WhatsApp messages, call logs, and communication history.\n\n"
            "Linked from: [[iZACH Brain]]\n\n"
            "---\n*Auto-generated by iZACH Smart Memory*\n"
        )
        with open(os.path.join(logs_dir, "Call-Messages-Logs.md"), "w", encoding="utf-8") as f:
            f.write(logs_md)

    except Exception as e:
        print(f"[SmartMemory] Hub node error: {e}")
...
def _sync_to_obsidian(entry: dict):
    try:
        ...
        folder = _OBS_FOLDERS.get(entry.get("category", "profile"), "User")
        ...
        hub  = _OBS_HUB.get(cat, "User")
        tags = [hub.lower(), cat, "izach-memory"]

        # Wikilinks — these create edges in graph view
        wikilinks = [f"[[{hub}]]", "[[iZACH Brain]]"]
        if entry.get("supersedes"):
            wikilinks.append(f"[[{entry['supersedes']}]]")
        if entry.get("superseded_by"):
            wikilinks.append(f"[[{entry['superseded_by']}]]")

        link_lines = []
        if entry.get("supersedes"):
            link_lines.append(f"- supersedes: [[{entry['supersedes']}]]")
        if entry.get("superseded_by"):
            link_lines.append(f"- superseded_by: [[{entry['superseded_by']}]]")
