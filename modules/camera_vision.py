...
def _groq_vision(b64: str, question: str) -> str | None:
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            if not choices:
                print(f"[VISION] Groq 200 but no choices: {str(data)[:200]}", flush=True)
                return None
            msg = (choices[0] or {}).get("message") or {}
            content = msg.get("content")
            if not content:
                print(f"[VISION] Groq 200 but no message.content: {str(data)[:200]}", flush=True)
                return None
            return content.strip()
        print(f"[VISION] Groq {resp.status_code}: {resp.text[:200]}", flush=True)
        return None
    except Exception as e:
        print(f"[VISION] Groq error: {e}", flush=True)
        return None
...
def _gemini_vision(b64: str, question: str) -> str | None:
    ...
    try:
        ...
        print(f"[VISION] Gemini error: {e}", flush=True)
        return None
...
def _openrouter_vision(b64: str, question: str) -> str | None:
    ...
    try:
        ...
        print(f"[VISION] OpenRouter {resp.status_code}: {resp.text[:200]}", flush=True)
        return None
    except Exception as e:
        print(f"[VISION] OpenRouter error: {e}", flush=True)
        return None
...
def _ask_vision(b64: str, question: str) -> str:
    ...
    if not (GROQ_KEY or any(GEMINI_KEYS) or OPENROUTER_KEY):
        print("[VISION] No vision API keys set at all (GROQ_VISION_KEY/GROQ_API_KEY, "
              "GEMINI_VISION_KEY_1-3/GEMINI_KEY_1-3, OPENROUTER_API_KEY all empty).", flush=True)
    for name, fn in [
        ("Groq", _groq_vision),
        ...
        if result:
            print(f"[VISION] Answered by {name}", flush=True)
            return result
        print(f"[VISION] {name} failed, trying next provider", flush=True)
